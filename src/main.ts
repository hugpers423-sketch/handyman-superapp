...
import { renderOpsPage } from './pages/OpsPage';
  ...
  app.innerHTML = `
    <div class="shell">
      ...
      <main id="main-content" role="main">
        ...
        <section class="page ${state.currentPage === 'ops' ? 'active' : ''}" id="ops" aria-labelledby="ops-heading">${renderOpsPage()}</section>
...
function attachEventListeners() {
  ...
  const assignButtons = document.querySelectorAll('[data-assign-urgent]');
  assignButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      const requestId = btn.getAttribute('data-assign-urgent');
      if (requestId) {
        assignUrgent(requestId, '2');
        btn.textContent = 'Asignada ✓';
        btn.classList.add('accepted');
        (btn as HTMLButtonElement).disabled = true;
        const statusCell = btn.closest('tr')?.querySelector('td:nth-child(3)');
        if (statusCell) {
          statusCell.innerHTML = '<span class="badge live">ROSA A. ASIGNADA</span>';
        }
      }
    });
  });
